...
async def get_writing_question(question_id: str) -> dict | None:
    client = await _client()
    data = (
        await client.table("writing_questions")
        .select("*")
        .eq("id", question_id)
        .maybe_single()
        .execute()
...
async def list_writing_samples(question_id: str) -> list[dict]:
    client = await _client()
    data = (
        await client.table("writing_samples")
        .select("*")
...
async def insert_writing_submission(payload: dict) -> dict:
    client = await _client()
    data = await client.table("writing_submissions").insert(payload).execute()
    return data.data[0]
...
async def get_writing_submission(submission_id: str) -> dict | None:
    client = await _client()
    data = (
...
async def insert_writing_feedback(payload: dict) -> dict:
    client = await _client()
    data = await client.table("writing_feedback").insert(payload).execute()
    return data.data[0]


async def delete_writing_feedback(feedback_id: str) -> None:
    client = await _client()
    await client.table("writing_feedback").delete().eq("id", feedback_id).execute()
